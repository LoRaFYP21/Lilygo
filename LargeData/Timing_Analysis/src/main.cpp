...
const char* wifi_ssid = "Dilsha";        // Replace with your WiFi SSID
const char* wifi_password = "iotlab123"; // Replace with your WiFi password
...
void initTimeSync() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (getLocalTime(&timeinfo)) {
      ...
      Serial.println();
      Serial.println("⚠  Time sync failed! Using relative timestamps.");
      timeInitialized = false;
    }
  } else {
    Serial.println();
    Serial.println("⚠  WiFi connection failed! Using relative timestamps.");
    timeInitialized = false;
  }
}
...
void formatSPIFFS() {
  Serial.println("⚠  WARNING: This will DELETE ALL files in SPIFFS!");
...
void showHelp() {
  ...
  Serial.println("FORMAT_SPIFFS - Format SPIFFS (⚠ DELETES ALL FILES!)");
  Serial.println("");
      ...
      if (Serial.available()) {
        ...
        if (fragSizeStr.length() > 0) {
          ...
          if (requestedSize >= 50 && requestedSize <= 250) {
            ...
            Serial.println("⚠  Invalid size! Using current: " + String(FRAG_CHUNK) + " bytes");
          }
        } else {
          Serial.println("✅ Using current fragment size: " + String(FRAG_CHUNK) + " bytes");
        }
      } else {
        Serial.println("⏱  Timeout! Using current fragment size: " + String(FRAG_CHUNK) + " bytes");
      }
